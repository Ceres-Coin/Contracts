...
// File contracts/staking/CeresStakingCRS.sol
...
contract CeresStakingCRS is CeresStakingRewards, IMintReceiver, IStakingReceiver, ERC20Votes {
    ...
    constructor(address _factory, address _asc, address _crs, address _stakerBook)
    ERC20("sCRS", "sCRS") ERC20Permit("sCRS") CeresBase(_factory) {
        asc = IERC20(_asc);
        crs = IERC20(_crs);
    ...
    function mintEarned(address account) public view override returns (uint256) {
        return _shareBalances[account] * (mintPerShareStored - accountMintPerSharePaid[account]) / 1e18 + mints[account];
    }
    ...
    function _afterStake(address account, uint256 shareAmount) internal override {
    ...
    function reinvestMint() public override nonReentrant updateMint(msg.sender) {
        ...
        if (mint > 0) {
            ...
            IStakingReceiver(stakingASC).notifyStaking(msg.sender, mint);
        }
    }
    ...
    function notifyMint(uint256 amountAsc, uint256 amountUsed) external override onlyCeresMiner {

        uint256 leftStaking = _totalStaking - amountUsed;
        require(crs.balanceOf(address(this)) >= leftStaking, "CeresStaking: Staking left balance is not enough!");
    ...
    function notifyStaking(address account, uint256 amount) external override updateMint(account) updateReward(account) {
        require(factory.isStaking(msg.sender) || msg.sender == factory.ceresReward() || msg.sender == factory.ceresMiner(),
            "CeresStaking: Sender is not the CeresStaking, CeresReward or CeresMiner!");
        require(_totalStaking + amount <= crs.balanceOf(address(this)), "CeresStaking: Notify amount exceeds balance!");
